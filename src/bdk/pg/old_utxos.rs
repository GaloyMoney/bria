...
impl OldUtxos {
    ...
    pub async fn persist(&self, utxo: &LocalUtxo) -> Result<(), bdk::Error> {
        sqlx::query!(
            r#"INSERT INTO bdk_old_utxos (keychain_id, tx_id, vout, utxo_json)
    ...
    pub async fn find_new_pending_tx(
        ...
        let utxos = sqlx::query!(
            r#"WITH utxo AS (
                 UPDATE bdk_old_utxos SET ledger_tx_pending_id = $1
                 WHERE keychain_id = $2 AND (tx_id, vout) in (
                   SELECT tx_id, vout FROM bdk_old_utxos
    ...
    pub async fn find_new_settled_tx(
        ...
        let utxos = sqlx::query!(
            r#"WITH utxo AS (
                 UPDATE bdk_old_utxos SET ledger_tx_settled_id = $1
                 WHERE keychain_id = $2 AND (tx_id, vout) in (
                   SELECT u.tx_id, vout
                     FROM bdk_old_utxos u
