#!/bin/bash
 ...
 export sidecar_digest=$(cat ./latest-provision-admin-sidecar-image/digest)
 ...
 yq -i e '.bria.provisionAdminSidecarImage.digest = strenv(sidecar_digest)' ./charts/${CHARTS_SUBDIR}/values.yaml
 yq -i e '.bria.image.git_ref = strenv(ref)' ./charts/${CHARTS_SUBDIR}/values.yaml
